...
int sys_fork()
{
	...
	//g. Assignem pid al fill i incrementem seguent pid a assignar
	fill->task.PID = ++pids;
	
	//i. 
	((unsigned long *)KERNEL_ESP(fill))[-0x13] = (unsigned long) 0;
	((unsigned long *)KERNEL_ESP(fill))[-0x12] = (unsigned long) &ret_from_fork;
	fill->task.kernel_esp = (unsigned int)&((unsigned long *)KERNEL_ESP(fill))[-0x13];
	
	
	//j. Afegim proces a la ready queue
	list_add_tail(&(fill->task.list), &readyqueue);

	//k. Retornem al pare PID del fill
